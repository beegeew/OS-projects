...
void process_arguments(int argc, char **argv)
{
	...
	//arguments
	arglist = (char**) malloc((argc-2)*sizeof(char *));
	...
	//Null terminate our command list to ensure that the arguments will pass to
	//the execv function properly
	arglist[i] = NULL;
}
...
void At_cmd(int delay, char* path)
{	
	//Sleep for delay and then run date command
	sleep(delay);
	
	printf("\nChild process: Current Shell = %s\n",path);
...
int main(int argc, char **argv)
{
	...
	if (argc < 2) {
		printf("Program Usage: At [+ for delay, @ for absolute time]00:00:00 [command] [arguments]\n");
		return 1;
	}
	...
	if (argc < 3) {
		...
		path = pathfind(getenv("PATH"),arglist[0],"rx");
		if (path != NULL)
	...
	//Print the process ID for the parent and children
	if ((At_cmd_id = fork()) == 0) 
	{
		printf("Child process: My pid = %d\nChild process: parent pid = %d\n",getpid(),getppid());
		//print then change the group ID
		printf("Child process: My original group id = %d\n",getpgrp());
		setpgid(At_cmd_id,getpid());
		printf("Child process: My new group id = %d\n",getpgrp());
		At_cmd(delay,path);
	} else {
		//Get and print the pid of the process, it's parent and the child pid
		printf("\nParent process: My pid = %d\nParent process: My group id: = %d\n",getpid(),getpgrp());
		printf("Parent process: Child's pid = %d\n",At_cmd_id);
		//Print the current shell, set the shell. 
		printf("Parent process: Current Shell = %s\n",getenv("SHELL"));
		putenv("SHELL=/usr/bin/sh");
		printf("Parent process: New Shell = %s\n",getenv("SHELL"));
	}
